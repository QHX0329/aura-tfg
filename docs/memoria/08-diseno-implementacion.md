...
El módulo gestiona el buzón de notificaciones del usuario (inbox) y el envío de notificaciones push a dispositivos móviles mediante el servicio **Expo Push Notifications**. Las entregas se procesan de forma asíncrona con tareas Celery para no bloquear las peticiones API.

#### Modelos

- **`Notification`** — Registro persistente de cada notificación en el inbox del usuario. Incluye `notification_type` (enumerado: `price_alert`, `new_promo`, `shared_list_changed`, `business_approved`, `business_rejected`), campos `title`/`body`, bandera `is_read`, `data` JSONB para payload variable, `action_url` para deep links (`bargain://...`) y `deleted_at` para soft-delete.
- **`UserPushToken`** — Token Expo Push registrado por cada dispositivo del usuario. La clave única `(user, device_id)` evita duplicados al re-instalar la app.

Los campos de preferencia de notificaciones del usuario (`notify_price_alerts`, `notify_new_promos`, `notify_shared_list_changes`) se almacenan directamente en el modelo `User` como campos booleanos nullable, permitiendo distinguir entre "no configurado aún" y "desactivado explícitamente".

#### API del inbox

| Método | Endpoint | Descripción |
|--------|----------|-------------|
| `GET` | `/api/v1/notifications/` | Lista el inbox del usuario (no eliminadas) |
| `POST` | `/api/v1/notifications/{id}/read/` | Marcar como leída |
| `POST` | `/api/v1/notifications/read-all/` | Marcar todas como leídas |
| `DELETE` | `/api/v1/notifications/{id}/` | Soft-delete (establece `deleted_at`) |
| `GET` | `/api/v1/notifications/unread-count/` | Contador para el badge de la app |

#### Tareas Celery

- **`send_push_notification(user_id, title, body, data)`** — Obtiene todos los `UserPushToken` activos del usuario y realiza la llamada al API de Expo Push. Gestiona errores `DeviceNotRegistered` limpiando tokens caducados automáticamente.
- **Hooks de eventos** — Funciones auxiliares que crean registros `Notification` y despachan la tarea push: `notify_price_alert`, `notify_new_promo`, `notify_shared_list_changed`, `notify_business_status`. Se invocan desde los módulos `prices`, `business` y `shopping_lists` cuando ocurren los eventos correspondientes.

### 8.4.7 Módulo de portal business (`apps/business`)

Este módulo implementa el portal de gestión de PYMEs y comercios locales, permitiéndoles gestionar su presencia en la plataforma, actualizar precios propios y crear promociones.

#### Modelos

- **`BusinessProfile`** — Perfil verificable asociado en relación 1:1 con un `User` de rol `business`. Incluye `tax_id` (CIF/NIF único), `address`, `website`, estado de verificación (`is_verified` + `rejection_reason`), y configuración de alertas de competidor (`price_alert_threshold_pct`, `last_competitor_alert_at`).
- **`Promotion`** — Promoción temporal de un producto en una tienda concreta. Admite descuento fijo (`flat`) o porcentual (`percentage`) sobre el campo `discount_value`, fecha de fin opcional, cantidad mínima (`min_quantity`) y contador de visualizaciones (`views`). Una restricción parcial única `(product, store) WHERE is_active` garantiza que no existan dos promociones activas para el mismo par producto-tienda.

La tienda (`Store`) se asocia al `BusinessProfile` a través de un FK nullable `business_profile_id`, añadido en la migración `0002_store_business_profile`, de forma que el perfil de negocio controla directamente la tienda sin requerir una tabla de relación intermedia.
...
La tarea `check_competitor_prices(business_profile_id)` se ejecuta periódicamente (Celery Beat, cada 6 horas). Compara los precios propios del negocio con los precios de competidores en el mismo radio geoespacial. Si detecta una diferencia superior al umbral configurado (`price_alert_threshold_pct`), crea una notificación al propietario y actualiza `last_competitor_alert_at` para evitar alertas repetidas en las siguientes 24 horas.
