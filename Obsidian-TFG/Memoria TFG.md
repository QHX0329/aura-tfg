---
title: Memoria TFG — BargAIn
tags:
  - tfg/memoria
  - tfg/documentacion
tipo: memoria
area: memoria
estado: en-progreso
secciones-completadas: 8
secciones-totales: 12
fuente:
  - docs/memoria/
  - TASKS.md
actualizado: 2026-04-05
relacionados:
  - "[[00 Dashboard/Inicio]]"
  - "[[06 Memoria/Mapa de Memoria]]"
  - "[[Fases/F6 — Pruebas, Deploy y Cierre]]"
---

# Memoria TFG — BargAIn

[[BargAIn — TFG|← Volver al proyecto]]
[[06 Memoria/Mapa de Memoria|Mapa de Memoria]]

## Estado de las secciones

| # | Título | Archivo | Estado |
|---|---|---|:---:|
| 01 | Introducción | `docs/memoria/01-introduccion.md` | ✅ |
| 02 | Objetivos | `docs/memoria/02-objetivos.md` | ✅ |
| 03 | Antecedentes y estado del arte | `docs/memoria/03-antecedentes.md` | ✅ |
| 04 | Comparativa con competidores | `docs/memoria/04-comparativa.md` | ✅ |
| 05 | Herramientas y justificación del stack | `docs/memoria/05-herramientas.md` | ✅ |
| 06 | Planificación y cronograma | `docs/memoria/06-planificacion.md` | ✅ |
| 07 | Requisitos | `docs/memoria/07-requisitos.md` | ✅ |
| 08 | Diseño e implementación | `docs/memoria/08-diseno-implementacion.md` | ✅ |
| 09 | Manual de usuario | `docs/memoria/09-manual-usuario.md` | ⬜ |
| 10 | Pruebas y cobertura | `docs/memoria/10-pruebas.md` | ⬜ |
| 11 | Conclusiones y trabajo futuro | `docs/memoria/11-conclusiones.md` | ⬜ |
| 12 | Bibliografía | `docs/memoria/12-bibliografia.md` | ⬜ |

## Navegación por notas puente

- [[06 Memoria/01 Introducción]]
- [[06 Memoria/02 Objetivos]]
- [[06 Memoria/03 Antecedentes y Estado del Arte]]
- [[06 Memoria/04 Comparativa con Competidores]]
- [[06 Memoria/05 Herramientas y Stack]]
- [[06 Memoria/06 Planificación y Cronograma]]
- [[06 Memoria/07 Requisitos]]
- [[06 Memoria/08 Diseño e Implementación]]
- [[06 Memoria/09 Manual de Usuario]]
- [[06 Memoria/10 Pruebas y Cobertura]]
- [[06 Memoria/11 Conclusiones y Trabajo Futuro]]
- [[06 Memoria/12 Bibliografía]]

## Fuente de verdad

- `docs/memoria/`
- `TASKS.md`


