...
\texttt{test\_distance\_ors.py} & optimizer & Cliente ORS (3 tests unitarios) \\
\hline
\end{tabular}
\end{table}
...
\subsection{NFR-02: Disponibilidad $\geq$ 99\% en staging}\label{subsec:nfr02}

El backend de staging se despleg\'o en Render.com (plan gratuito) con la siguiente arquitectura:
\begin{itemize}
  \item Web Service: Django Gunicorn con 3 workers
  \item Redis: broker Celery + cach\'e Google Places
  \item Celery worker + Celery beat: tareas as\'incronas independientes
\end{itemize}

El health check se configura en \texttt{GET /api/v1/health/} respondiendo HTTP 200. Los reinicios
autom\'aticos ante fallos se gestionan por la plataforma Render.

\subsection{NFR-04: Usabilidad WCAG 2.1 AA y m\'aximo 3 taps}\label{subsec:nfr04}
...
\subsection{NFR-05: Escalabilidad para 10.000 usuarios}\label{subsec:nfr05}
...
\hline
Tests unitarios backend & \textasciitilde{}40 & Todos pasan \\
Tests de integraci\'on backend & \textasciitilde{}17 archivos & Todos pasan \\
Tests unitarios ORS (\texttt{test\_distance\_ors.py}) & 3 & Todos pasan \\
Tests E2E Playwright & 4 flujos & Todos pasan \\
UAT manual m\'ovil & 5 flujos & Verificados \\
NFR-02 (Disponibilidad) & 1 & Per\'iodo de monitorizaci\'on en curso \\
NFR-04 (Usabilidad) & 3 flujos & Verificados (3 taps m\'ax.) \\
NFR-05 (Escalabilidad) & Justificaci\'on arquitectural & Documentado \\
\hline
\end{tabular}
\end{table}
...
granularidad. La cobertura de tests backend supera el 80\% sobre los m\'odulos cr\'iticos, y los
4 flujos E2E automatizados con Playwright verifican que el sistema integrado funciona correctamente
de extremo a extremo. Los requisitos no funcionales de disponibilidad y usabilidad quedan
documentados con evidencia real del entorno de staging.
