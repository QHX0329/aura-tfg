...
\begin{itemize}
  ...
  \item \textbf{F4:} frontend base y avanzado --- autenticaci\'on, listas, cat\'alogo, mapa,
    notificaciones, perfil, portal business y Places enrichment.
  \item \textbf{F5:} sistema de optimizaci\'on multicriterio (OR-Tools + OpenRouteService),
    ...
    + fuzzy matching), asistente LLM (Google Gemini 2.0 Flash) y wiring de pantallas m\'oviles a
    endpoints reales.
...
\begin{itemize}
  ...
  \item \textbf{Asistente LLM con guardrails de dominio:} Gemini 2.0 Flash con restricci\'on
  ...
  \item \textbf{Verificaci\'on automatizada:} suite de tests backend (unitarios + integraci\'on,
    \textgreater80\% cobertura), tests E2E con Playwright (4 flujos cr\'iticos) y UAT manual en
    iPhone.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{NFR-05 sin load test real:} la justificaci\'on de escalabilidad a 10.000 usuarios
